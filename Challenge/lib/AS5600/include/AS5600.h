...
#define I2C_AS5600_ADDRESS_DEFAULT 0x36

// =============================================================================
// AS5600_DEG_PER_COUNT — used by Encoders.cpp to convert raw to degrees
// =============================================================================
constexpr float AS5600_DEG_PER_COUNT = 360.0f / 4096.0f;  // 12-bit resolution
...
struct AS5600_CONF
{
    ...
    void SetPowerMode          (AS5600_CONF_PM   pm)   { _conf = (_conf & ~0x03)         | ( pm   & 0x03); }
    void SetHysteresis         (AS5600_CONF_HYST hyst) { _conf = (_conf & ~(0x03 << 2))  | ((hyst & 0x03) << 2); }
    void SetOutputStage        (AS5600_CONF_OUTS outs) { _conf = (_conf & ~(0x03 << 4))  | ((outs & 0x03) << 4); }
    void SetPWMFrequency       (AS5600_CONF_PWMF pwmf) { _conf = (_conf & ~(0x03 << 6))  | ((pwmf & 0x03) << 6); }
    void SetSlowFilter         (AS5600_CONF_SF   sf)   { _conf = (_conf & ~(0x03 << 8))  | ((sf   & 0x03) << 8); }
    void SetFastFilterThreshold(AS5600_CONF_FTH  fth)  { _conf = (_conf & ~(0x07 << 10)) | ((fth  & 0x07) << 10); }
...
{
public:
    // Constructor completo: configura e instala el driver I2C
    AS5600_i2c(gpio_num_t sda, gpio_num_t scl,
           i2c_port_t i2c_num = I2C_NUM_0,
           uint8_t    address = I2C_AS5600_ADDRESS_DEFAULT)
    : i2c_num(i2c_num), i2c_address(address)
{
    i2c_driver_initialize(sda, scl, i2c_num);
}

    // Constructor ligero: driver ya instalado (segundo sensor en el mismo bus)
    AS5600_i2c(i2c_port_t i2c_num, uint8_t address = I2C_AS5600_ADDRESS_DEFAULT)
        : i2c_num(i2c_num), i2c_address(address) {}

    // Constructor por defecto: para usar como miembro de clase antes del setup
    AS5600_i2c() : i2c_num(I2C_NUM_0), i2c_address(I2C_AS5600_ADDRESS_DEFAULT) {}
