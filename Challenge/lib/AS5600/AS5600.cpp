...
// =============================================================================
// i2c_driver_initialize  —  three bugs fixed vs original:
//   1. i2c_param_config now uses i2c_num instead of hardcoded I2C_NUM_0
//   2. param_config runs BEFORE driver_install (original had them swapped)
//   3. driver_install tolerates ESP_ERR_INVALID_STATE so two encoders can
//      share a bus without crashing on the second install call
// =============================================================================
...
#pragma GCC diagnostic pop
    ...
    // FIX 1: use i2c_num, not hardcoded I2C_NUM_0
    ESP_ERROR_CHECK(i2c_param_config(i2c_num, &i2c_config));

    // FIX 2+3: install after config; ignore "already installed" error
    esp_err_t err = i2c_driver_install(i2c_num, I2C_MODE_MASTER, 0, 0, 0);
    if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
        ESP_ERROR_CHECK(err);
    }

    ESP_LOGI(TAG, "I2C port %d initialised — SDA:%d SCL:%d @ %d Hz",
             i2c_num, sda, scl, I2C_SPEED_HZ);
}
...
esp_err_t AS5600_i2c::write_MPOS(uint16_t mpos)
    { return write_registr(AS5600_REG_MPOS, mpos); }

esp_err_t AS5600_i2c::read_MANG(uint16_t& mang)
    { return read_registr(AS5600_REG_MANG, mang); }

esp_err_t AS5600_i2c::write_MANG(uint16_t mang)
    { return write_registr(AS5600_REG_MANG, mang); }
...
esp_err_t AS5600_i2c::read_RAWANGLE(uint16_t& angle)
    { return read_registr(AS5600_REG_RAWANGLE, angle); }
...
const char* CONF_PM_String(AS5600_CONF_PM pm) {
    switch (pm) {
        case PM_NOM:  return "NOM";
        case PM_LPM1: return "LPM1";
        case PM_LPM2: return "LPM2";
        case PM_LPM3: return "LPM3";
...
const char* CONF_HYST_String(AS5600_CONF_HYST hyst) {
    switch (hyst) {
        case HYST_OFF:  return "OFF";
        case HYST_1LSB: return "1LSB";
        case HYST_2LSB: return "2LSB";
        case HYST_3LSB: return "3LSB";
        default:        return "Unknown";
    }
}
...
const char* CONF_OUTS_String(AS5600_CONF_OUTS outs) {
    switch (outs) {
        case OUTS_ANALOG_FULL: return "ANALOG_FULL";
        case OUTS_ANALOG:      return "ANALOG";
        case OUTS_PWM:         return "PWM";
        default:               return "Unknown";
    }
}
...
const char* CONF_PWMF_String(AS5600_CONF_PWMF pwmf) {
    switch (pwmf) {
        case PWMF_115: return "115 Hz";
        case PWMF_230: return "230 Hz";
        case PWMF_460: return "460 Hz";
        case PWMF_920: return "920 Hz";
        default:       return "Unknown";
    }
}
...
const char* CONF_FTH_String(AS5600_CONF_FTH fth) {
    switch (fth) {
        case FTH_0:  return "Slow filter only";
        case FTH_6:  return "6";
        case FTH_7:  return "7";
        case FTH_9:  return "9";
        case FTH_18: return "18";
        case FTH_21: return "21";
        case FTH_24: return "24";
        case FTH_10: return "10";
        default:     return "Unknown";
    }
}